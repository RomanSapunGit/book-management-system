...
async def test_register_duplicate_email_409(client):
    ...
    r = await client.post(
        "/auth/register", json={"email": _USER["email"].upper(), "password": _USER["password"]}
    )
    assert r.status_code == 409
...
async def test_login_wrong_password_401(client):
    await client.post("/auth/register", json=_USER)
    r = await client.post(
        "/auth/login", json={"email": _USER["email"], "password": "wrong-password-but-valid-length"}
    )
...
async def test_access_token_works(client):
    ...
    r = await client.post(
        "/authors", json={"name": "T"}, headers={"Authorization": f"Bearer {token}"}
    )
    assert r.status_code == 201
...
async def test_bad_access_token_401(client):
    r = await client.post(
        "/authors", json={"name": "T"}, headers={"Authorization": "Bearer not.a.real.jwt"}
    )
    assert r.status_code == 401
